...
The Final Check tab's Final build of this widget is complete and signed off by the project owner. Version badge set to v2.0 (`FC_VERSION[3]`); title badges: "Final" and "Jo design". The Final renders by default; the earlier A/B/C options stay reachable from the section's design-option switch. Summary of what shipped:

**Composition (Jo's design, ported wholesale):** the Final is Jo Lopez's payroll widget carried into the Final Check tab verbatim (the additive prF block beside `WRENDER[3]` in `Dashboard Widget Mockups.html`; the A/B/C branches are untouched): her two views (Table as the default plus a Donut; at Detail both panels render together and the view toggle is hidden), her scope system (All distributions / By pay type, plus drilling into a single distribution in place with a context line saying what is being viewed, not a modal and not a page jump), her sortable tables in every scope, her blue-ramp donut with legend hover sync, her empty state that keeps its toolbar visible, her honest export stubs with toast feedback, instant client-side recompute on every control (this widget has no fetch at all), and her full accessibility layer. The ONE change from her design is the period preset row, recorded under Owner decisions below.

**Owner decisions layered on Jo's design (v2 deltas):** the period presets are now This month (default) / This period / This quarter / This year / All time. This year is the rolling last 12 months back from the as-of date, per the Time Window Module's window definition; All time removes the date bound entirely. This year and All time were added the same day, per direct instruction, after the initial three-preset version. Jo's Custom From/To date row is kept and stays first in the control, including its focus-restore behaviour, per Jo's rule and the supporting SME point on custom date ranges. Window selection is scope-only: there is NO grain/interval toggle (the widget adopts the module's window definitions only, not the full module contract), and NO prior-period comparison anywhere in the Final, respecting sign-off flag F3, which rejected current-vs-prior comparison for this widget.

**Mock data notes:** the seeded payroll runs were extended from 12 to 13, anchored to Jul 31 2026. In the mock, This month and This period coincide because the mock's fiscal calendar equals calendar months; they remain distinct API parameters all the same (each preset maps to its own request parameter, never a client-side alias of another).

**Verification:** 213-assertion per-widget Node DOM-shim driver, including 3-timezone runs and the focus-restore click path; browser-faithful CSS parse check (0 dropped rules); final-check-rules.py 0 HIGH; W01 and W02 regression drivers green; the Dashboard tab byte-identical before and after the build. `FC_VERSION[3]` = 2.0.

**New evidence (filed the same day):** the project owner's technical analysis at `Step 5 - API documents/Payroll Distributions/Payroll Distributions - Pay Type Breakdown Analysis (proof).html` proves the pay-type breakdown is fully supported by the database: "A drill-down endpoint can be built without any schema changes." `PR_HistoryCompensation` carries `SubType` (fixed pay-type codes 1 to 10: Regular, OverTime, DoubleTime, Holiday, Other, Vacation, Sick, Personal, Misc., Other Pay) plus `Hours`, `Rate` and `Amount` on every compensation line, while the labels this widget groups by are org-defined distribution names from `PR_CompensationDistribution.Name`. That resolves the sign-off contradiction over the pay-type list (flag F4): Jo's live audit (org-defined free-text categories such as "AA Aid" and "Administration Staff") and the project's documented fixed pay-type list are BOTH true, at different levels of the same hierarchy: level 2 is the org-defined distributions, level 4 is the fixed `SubType` pay types, and both levels live in `PR_HistoryCompensation`. One nuance from the analysis: SubTypes 6 to 9 (Vacation, Sick, Personal, Misc) have org-configurable display names resolved from company settings. The reconciliation file's finding 1 has been updated to Resolved with evidence accordingly; this also gives the "Also raised, and also dropped for now" deeper-breakdown question above its answer: the backend CAN support drilling from a Distribution into further detail, with no schema changes.
