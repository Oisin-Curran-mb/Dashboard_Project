...
### Rule 12 — Final versions use Jo's three-size model; Dashboard mock options A/B/C do not (added 2026-07-27)

This rule applies **only to Final versions** (the composed `opt==='F'` builds in the Final Check tab, made by the `build-final-widget` skill). It does NOT change anything for the Dashboard tab's design options A/B/C, which keep Rules 6 and 9 exactly as written above (KPI/Medium/Large mandatory, Small optional-with-documented-decision). The two size models coexist deliberately: mocks explore with the project's own grid; Finals conform to the target platform's shell.

For every Final version:
- **Three sizes only, using Jo's terms in her order: Glance, Explore, Detail.** There is no Small. Mapping to this project's slot letters: `k` = Glance, `m` = Explore, `l`/`x` = Detail (the `s` slot is hidden while a Final is active and must still render safely as the mid tier if invoked).
- **Proportions match her demo's grid tiers** (12-column grid, 48px rows, 16px gaps): Glance = 3 columns × 176px; Explore = 6 columns × 496px; Detail = 12 columns × 560px. In the Final Check tab these are applied as aspect-ratio overrides scoped to F mode (`fc-fmode`), never as edits to the shared slot dimensions A/B/C use.
- **Naming decision (corrected 2026-07-27, per direct instruction):** the authoritative mapping is her demo's own `SIZE_LABEL`/`DEPTH_LABEL` code: kpi = Glance, mid = **Explore**, largest = **Detail**. An earlier same-day version of this rule followed the order her spec documents happen to list the names in (Glance/Detail/Explore) and had the top two reversed; the project owner confirmed the demo's mapping is the correct one. The ladder is always written **Glance, Explore, Detail** and no other order.
- W01 Budget Compared to Actual (built 2026-07-27) is the reference implementation of this rule.
