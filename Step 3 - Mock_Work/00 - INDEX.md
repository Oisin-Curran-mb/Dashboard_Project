...
| `Final Check - Items Needing Your Review.md` | Live | Active review checklist, real open checkboxes. **As of 2026-07-27 also carries a dated "Conflicts found during the Step 4 template upgrade" section** — cross-source contradictions (W17 % Due definition, W05 fourth KPI tile, W04 KPI-size filter, plus the W04/W05/W06 view-drift note) recorded as decision questions. Checking one off means the decision was made AND the Step 4 doc was updated to match. The `build-final-widget` skill treats an open conflict here as a build blocker for that widget. |
...
| `final-check-rules.py` | **Live tool (added 2026-07-27)** | Companion to `check-rules.py` — does NOT replace it. Verifies one widget's FINAL build (the `fc-widget-N` section + `WRENDER[N]` + mock data) rather than linting the Dashboard tab's options. Checks F1–F9: block completeness (incl. whether an `opt==='F'` final branch exists yet), node syntax gate, doc-vs-code against the widget's Step 4 doc (`--step4` path), values-in-DOM, colour-only signals, fixed-date literals, em-dash sweep, empty-state guard, and the Step 4 Sign-off Readiness blocking-row gate. Run: `python3 final-check-rules.py "Dashboard Widget Mockups.html" --widget N --step4 "../Step 4 - Widget Final Design/WNN - Name.md"`; `--cleanup` scans the whole file for orphans (fc section without WRENDER, orphaned series, missing closing comments). Exit 1 on HIGH only. Adapted from Jo Lopez's verification method; the static half of the `build-final-widget` skill's verify phase (the skill's per-widget Node DOM-shim driver is the dynamic half). |
...
7. **This folder now has a 4-skill pipeline writing to it directly.** The original 3 were built and chained 2026-07-23, and a 4th was added 2026-07-27: `build-final-widget` — owns the **Final Check tab** (`fc-widget-N` sections) the way Create Mock Designs owns the Dashboard tab's option cards, and the two never edit each other's territory. **As of 2026-07-28 it has produced built Finals for W01, W02, W03, W04, and W05** — each a Jo-design `opt==='F'` branch in `Dashboard Widget Mockups.html` (A/B/C untouched, tagged v2.0 with "Final" + "Jo design" badges), verified by `final-check-rules.py` + a per-widget Node driver. The Final Check tab now renders these five as their real final versions, not design options. It composes a widget's final version interactively with the project owner (bits of options A/B/C + the upgraded Step 4 doc + Jo's reference build in the `design-sandbox` Widget Container Demo where one exists), builds it as a new `opt==='F'` branch in `WRENDER[N]` (A/B/C untouched), verifies with `final-check-rules.py` plus a per-widget Node DOM-shim driver that executes the real render, and carries its own fix loop capped at 3 cycles. It refuses to build past an open conflict in `Final Check - Items Needing Your Review.md`, a blocks-build row in the widget's Step 4 Sign-off Readiness table, or a Disputed Step 6 finding. The original chain, built and chained 2026-07-23: `Create Mock Designs` (edits `Dashboard Widget Mockups.html`, re-syncs `mock-data.master.js`, appends dated entries to the relevant `Widget_Specs/WNN-Name.md`, then automatically hands off to Verify) → `Verify Mock Designs` (writes/keeps `Verify Findings.md`, per the row above, and calls Fix itself if it finds anything) → `Fix Mock Designs` (patches only what's flagged, clears resolved lines from that file, never deletes it). Verify caps itself at 3 calls to Fix per cycle — if a 4th check still finds something, it stops and reports to the project owner rather than continuing. If you're test-running one of these against the real files here, back up first and **remove your own backup folder the same session once the result is locked in** — don't leave a `_dry-run-backups`-style folder sitting in this project directory (one existed briefly for a W10 test on 2026-07-23 and was cleaned up the same day).
