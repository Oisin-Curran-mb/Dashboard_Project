...
ok('1 table view is the default', FAF_STATE.view==='assets');
var tg=fafViewToggle(FAF_STATE);
ok('2 toggle has 2 segments', (tg.match(/data-faf="view"/g)||[]).length===2);
ok('3 Asset Detail is first in the toggle', tg.indexOf('data-v="assets"')<tg.indexOf('data-v="donut"'));
ok('4 Asset Detail is the ON segment by default', /class="vt on"[^>]*data-v="assets"/.test(tg));
ok('5 fafGap gone', typeof fafGap==='undefined');
ok('6 fafOpenPanel gone', typeof fafOpenPanel==='undefined');
ok('7 FAF_OPEN gone', typeof FAF_OPEN==='undefined');

var combos=[];
['kpi','wide','xwide'].forEach(function(sz){ ['assets','donut'].forEach(function(v){ combos.push([sz,v]); }); });
combos.forEach(function(c){
  var h=fafContent(W({size:c[0],view:c[1]}));
  var n=c[0]+'/'+c[1];
  ok('8 no gap note at '+n, !has(h,'faf-gap'));
  ok('9 no open panel at '+n, !has(h,'faf-open'));
  ok('10 no "Settled by" at '+n, !has(h,'Settled by'));
  ok('11 no "not settled" heading at '+n, !has(h,'are not settled'));
  ok('12 no context line at '+n, !has(h,'Current book values'));
  ok('13 no faf-ctx at '+n, !has(h,'faf-ctx'));
  ok('14 renders at '+n, h.length>50);
  ok('15 no em dash at '+n, h.indexOf('\u2014')<0);
});
// header shape: one row, chips + toggle + download
var hw=fafContent(W({size:'wide',view:'assets'}));
ok('16 chiprow present', has(hw,'faf-chiprow'));
ok('17 three chips', (hw.match(/class="filter-chip/g)||[]).length===3);
ok('18 toggle present', has(hw,'faf-vtoggle'));
ok('19 download present', has(hw,'data-faf="download"'));
ok('20 no dep-hd-num row', !has(hw,'dep-hd-num'));
ok('21 table renders in the default view', has(hw,'role="table"'));
// unimplemented dimension still an honest empty frame, no invented figure
/* The REAL dimension keys, read from FAF_DIMS rather than guessed: cls, bldg, room
   are impl:true; acctAsset, acctAccum, acctExp are impl:false. An unknown key is
   correctly coerced to Class by fafDimDef, which is why inventing keys in a test
   produces Class data and looks like a bug. */
['acctAsset','acctAccum','acctExp'].forEach(function(d){
  var hu=fafContent(W({size:'wide',dim:d}));
  var bare=hu.replace(/aria-label="[^"]*"/g,'').replace(/title="[^"]*"/g,'');
  ok('22 '+d+' renders a state', hu.length>50);
  ok('23 '+d+' invents no figure', !/\$[\d,]+/.test(bare));
  ok('24 '+d+' renders no asset table', !has(hu,'role="table"'));
  ok('25 '+d+' has no groups', fafGroupNames(W({dim:d})).length===0);
});
['cls','bldg','room'].forEach(function(d){
  ok('26 '+d+' does have groups', fafGroupNames(W({dim:d})).length>0);
});
ok('27 Building groups differ from Class groups',
   JSON.stringify(fafGroupNames(W({dim:'bldg'})))!==JSON.stringify(fafGroupNames(W({dim:'cls'}))));
ok('28 Glance still ignores selections',
   fafContent(W({size:'kpi'}))===fafContent(W({size:'kpi',dim:'bldg',measure:'cost'})));
