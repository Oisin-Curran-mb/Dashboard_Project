...
function rows(h){return (h.match(/class="wt-row faf-arow/g)||[]).length;}

// 1. legacy headings
var h=fafContent(W({size:'wide',view:'assets'}));
['Capitalized','Cost','Depreciable','Accumulated','Net'].forEach(function(x){
  ok('1 heading "'+x+'" present', new RegExp('columnheader[^>]*>'+x.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'<').test(h));
});
ok('2 no "Accum. Depn"', !has(h,'Accum. Depn'));
ok('3 no "Net Value" heading', !/columnheader[^>]*>Net Value</.test(h));

// 2. enough data to page
ok('4 dataset is large', FAF_ASSETS.length>=50);
var all=fafSortedAssets(W({view:'assets'}));
ok('5 page size is 10', FAF_PAGE_SIZE===10);

// 3. pagination
var big=null;
['Audio Visual','Buildings','Office Equipment','Vehicles'].forEach(function(g){
  var w=W({size:'wide',view:'assets',dim:'cls',group:g});
  if(fafSortedAssets(w).length>FAF_PAGE_SIZE && !big) big=w;
});
ok('6 a group exceeds one page', big!==null);
if(big){
  var t1=fafContent(big);
  ok('7 pager rendered', has(t1,'faf-pager'));
  ok('8 Next present', has(t1,'data-faf="page-next"'));
  ok('9 Prev disabled on page 1', /data-faf="page-prev"[^>]*disabled/.test(t1));
  /* rows() counts class="wt-row faf-arow", which matches body rows and the totals row
     but NOT the header, whose class list is "wt-row wt-head faf-arow". So a full page
     is PAGE_SIZE body rows + 1 totals row. role="row" is the one that counts all
     three, at PAGE_SIZE + 2. */
  ok('10 page 1 shows exactly the page size', rows(t1)===FAF_PAGE_SIZE+1);
  ok('10b and three rowgroups worth of rows', (t1.match(/role="row"/g)||[]).length===FAF_PAGE_SIZE+2);
  ok('10c the last page holds the remainder',
     (function(){var n=fafPageCount(big),lastW=W(big);lastW.page=n-1;
       var rem=fafSortedAssets(big).length-(n-1)*FAF_PAGE_SIZE;
       return rows(fafContent(lastW))===rem+1;})());
  var p2=W(big); p2.page=1;
  var t2=fafContent(p2);
  ok('11 page 2 renders different rows', t1!==t2);
  ok('12 Prev enabled on page 2', !/data-faf="page-prev"[^>]*disabled/.test(t2));
  // THE RULE: totals must not change between pages
  function tot(x){var m=x.match(/aria-label="Total Net[^"]*"/); return m?m[0]:null;}
  ok('13 the totals row is identical on both pages', tot(t1)===tot(t2) && tot(t1)!==null);
  ok('14 the totals row counts the WHOLE group, not the page',
     new RegExp('across '+fafSortedAssets(big).length+' asset').test(tot(t1)||''));
  ok('15 aria-rowcount is the full set', has(t1,'aria-rowcount="'+(fafSortedAssets(big).length+2)+'"'));
  // clamping
  var pOver=W(big); pOver.page=99;
  ok('16 an out-of-range page clamps to the last', fafPageIndex(pOver)===fafPageCount(big)-1);
  var pNeg=W(big); pNeg.page=-5;
  ok('17 a negative page clamps to the first', fafPageIndex(pNeg)===0);
}
// a small group shows no pager at all
var small=W({size:'wide',view:'assets',dim:'cls',group:'Kitchen Equipment'});
if(fafSortedAssets(small).length<=FAF_PAGE_SIZE) ok('18 no pager when it fits', !has(fafContent(small),'faf-pager'));
else P++;

// 4. download: icon only, in its own right-aligned row
ok('19 action row present', has(h,'faf-actionrow'));
ok('20 download is in it', h.indexOf('faf-actionrow')<h.indexOf('data-faf="download"'));
ok('21 no "Download" text label', !/>Download</.test(h));
ok('22 icon-only button has an accessible name', /data-faf="download"[^>]*aria-label="[^"]+"/.test(h));
ok('23 download is out of the header toggle', h.indexOf('data-faf="download"')>h.indexOf('dep-hd-toggle'));
ok('24 action row on the donut too', has(fafContent(W({size:'wide',view:'donut'})),'faf-actionrow'));

// nothing regressed
ok('25 Glance has no action row', !has(fafContent(W({size:'kpi'})),'faf-actionrow'));
ok('26 Glance still ignores selections', fafContent(W({size:'kpi'}))===fafContent(W({size:'kpi',measure:'cost'})));
ok('27 measure chip still donut-only', !has(h,'Measure:') && has(fafContent(W({size:'wide',view:'donut'})),'Measure:'));
ok('28 no em dash', !['assets','donut'].some(function(v){return ['kpi','wide','xwide'].some(function(s){return fafContent(W({size:s,view:v})).indexOf('\u2014')>=0;});}));
