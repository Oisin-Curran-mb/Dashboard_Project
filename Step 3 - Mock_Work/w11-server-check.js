...
function fafPager(w,res){
  /* Reads the response only. The client does not know how many pages there are until
     the server says so, which is the point. */
  if(res.totalCount<=FAF_PAGE_SIZE) return '';
  var i=res.pageIndex,n=res.pageCount;
  var from=i*FAF_PAGE_SIZE+1,to=Math.min(res.totalCount,(i+1)*FAF_PAGE_SIZE);
  ...
  return '<div class="faf-pager">'+
    '<span class="faf-pgcount">'+from+' to '+to+' of '+res.totalCount+'</span>'+
    btn('page-prev','chevron_left','Previous page, refetches from the server',i===0)+
    '<span class="faf-pgnum">Page '+(i+1)+' of '+n+'</span>'+
    btn('page-next','chevron_right','Next page, refetches from the server',i>=n-1)+
  '</div>';
}
/* 2026-09-03, owner instruction: THE SORT DOES NOT HAPPEN IN THE BROWSER.
   Each column sort is a server request, which is also what this widget's pagination
   already forces: the client holds at most one page and cannot order a set it does not
   have. So sort and page are REQUEST PARAMETERS, not client operations.

   fafServerQuery is the stand-in for that endpoint. It is deliberately shaped like the
   response a paged, server-sorted grid must return, and it is the ONLY thing the table
   renders from:
       { rows, totalCount, totals, pageIndex, pageCount }
   rows        the requested page ONLY, already ordered by the server
   totalCount  the size of the whole filtered group, not the page
   totals      the per-measure totals over the WHOLE filtered group
   pageIndex   the clamped page actually served
   pageCount   how many pages exist

   Everything inside this function stands for work the API does. The renderers below
   must never sort, slice or total; if they did, the mock would be lying about where the
   work happens and the real build would be written against the wrong contract.

   The corresponding request is:
       GET /api/dashboard/fixed-assets/grid
           ?valueType={dim}&valueId={group}&sortBy={key}&sortDir={asc|desc}
           &page={n}&pageSize={FAF_PAGE_SIZE}
   sortBy must be whitelisted server side, and the order must end in a unique
   tiebreaker (tag) or paging can show or skip the same row. */
var FAF_PAGE_SIZE=10;
var FAF_SORTABLE=['tag','nm','cap','cost','depr','accum','net'];
function fafServerQuery(w){
  /* --- everything from here to the return is SERVER-SIDE work --- */
  var d=fafDimDef(w.dim),g=fafCurrentGroup(w);
  if(g===null) return {rows:[],totalCount:0,totals:{},pageIndex:0,pageCount:1};
  var set=fafDataset(w).filter(function(a){return fafGroupOf(a,d.k)===g;});
  var parts=(w.sort||'tag-asc').split('-'),key=parts[0],dir=parts[1]==='asc'?1:-1;
  if(FAF_SORTABLE.indexOf(key)<0) key='tag';            /* whitelist, as the API must */
  ...
  set.sort(function(a,b){
    ...
    if(r===0) r=a.tag.localeCompare(b.tag);              /* unique tiebreaker */
    return dir*r;
  });
  var totals={};
  FAF_MEASURES.forEach(function(m){
    totals[m.k]=set.reduce(function(s,a){return s+fafValue(a,m.k);},0);
  });
  var pageCount=Math.max(1,Math.ceil(set.length/FAF_PAGE_SIZE));
  var pi=w.page||0; if(pi<0) pi=0; if(pi>pageCount-1) pi=pageCount-1;
  return {
    rows:set.slice(pi*FAF_PAGE_SIZE,(pi+1)*FAF_PAGE_SIZE),
...
/* Takes the SERVER'S totals and count. It must not reduce over the rows it was given,
   because those are one page. This is the function that would silently produce a wrong
   number if the work drifted back into the browser. */
function fafAssetTotalRow(w,res){
  var cells=fafMeasureOrder().map(function(m){
    var v=res.totals[m.k]||0;
    var exact='Total '+m.l+', '+fafMoneyFull(v)+' across '+res.totalCount+' asset'+(res.totalCount===1?'':'s');
    return '<span class="faf-c" role="cell" title="'+fafEsc(exact)+'" aria-label="'+fafEsc(exact)+'">'+fafMoneyShort(v)+'</span>';
  }).join('');
  return '<div class="wt-row faf-arow faf-totalrow" role="row">'+
    '<span class="faf-tag" role="rowheader">Total</span>'+
    '<span class="faf-nm">'+res.totalCount+' asset'+(res.totalCount===1?'':'s')+'</span>'+
    cells+'</div>';
}
function fafAssetTable(w){
  var g=fafCurrentGroup(w);
  if(g===null) return fafUnimplemented(w);
  /* ONE request per render. The renderers below sort nothing, slice nothing and total
     nothing; they display what came back. */
  var res=fafServerQuery(w),d=fafDimDef(w.dim);
  var body=res.rows.length
    ? res.rows.map(function(a){return fafAssetRow(w,a);}).join('')
    : '<div class="wt-row faf-arow" role="row"><span class="faf-nm" role="cell">No assets in this group.</span></div>';
  var aria='Assets in '+d.l+' '+g+', '+res.totalCount+' asset row'+(res.totalCount===1?'':'s')+', 7 columns';
  return '<div class="faf-tblwrap">'+
    '<div class="scroll faf-tscroll" role="table" aria-label="'+fafEsc(aria)+'" aria-rowcount="'+(res.totalCount+2)+'" aria-colcount="7">'+
      '<div class="faf-tbl">'+
        '<div class="faf-rowgroup" role="rowgroup">'+fafAssetHead(w)+'</div>'+
        '<div class="faf-rowgroup" role="rowgroup">'+body+'</div>'+
        '<div class="faf-rowgroup" role="rowgroup">'+fafAssetTotalRow(w,res)+'</div>'+
      '</div>'+
    '</div>'+
    fafPager(w,res)+
  '</div>';
...
function fafOnClick(e){
  ...
  if(a==='page-prev'){ w.page=Math.max(0,fafServerQuery(w).pageIndex-1); fafRerender(); return; }
  if(a==='page-next'){ var q=fafServerQuery(w); w.page=Math.min(q.pageCount-1,q.pageIndex+1); fafRerender(); return; }
...
// --- the browser must not sort, slice or total ---
ok('1 no client sort helper', typeof fafSortedAssets==='undefined');
ok('2 no client page slicer', typeof fafPageRows==='undefined');
ok('3 the server query exists', typeof fafServerQuery==='function');
var SRC=fafAssetTotalRow.toString()+fafAssetTable.toString()+fafPager.toString()+fafAssetRow.toString()+fafAssetHead.toString();
ok('4 no renderer calls .sort(', SRC.indexOf('.sort(')<0);
ok('5 no renderer calls .slice( for paging', SRC.indexOf('.slice(')<0);
ok('6 the totals row does not reduce over rows', fafAssetTotalRow.toString().indexOf('.reduce(')<0);
ok('7 the totals row reads res.totals', fafAssetTotalRow.toString().indexOf('res.totals')>=0);
ok('8 the pager reads res.pageCount', fafPager.toString().indexOf('res.pageCount')>=0);

// --- the response shape is what a paged, server-sorted grid must return ---
var big=null;
['Audio Visual','Buildings','Office Equipment'].forEach(function(g){
  var w=W({view:'assets',dim:'cls',group:g});
  if(!big && fafServerQuery(w).totalCount>FAF_PAGE_SIZE) big=w;
});
ok('9 found a multi-page group', big!==null);
var r=fafServerQuery(big);
['rows','totalCount','totals','pageIndex','pageCount'].forEach(function(k){
  ok('10 response carries '+k, r[k]!==undefined);
});
ok('11 rows is one page only', r.rows.length===FAF_PAGE_SIZE);
ok('12 totalCount is the whole group', r.totalCount>r.rows.length);
ok('13 totals cover all five measures', FAF_MEASURES.every(function(m){return typeof r.totals[m.k]==='number';}));
ok('14 totals are over the whole group, not the page', (function(){
  var pageSum=r.rows.reduce(function(s,a){return s+fafValue(a,'net');},0);
  return r.totals.net!==pageSum;
})());

// --- sort is a request parameter and the server applies it ---
FAF_SORTABLE.forEach(function(k){
  var asc=fafServerQuery(W(Object.assign({},big,{sort:k+'-asc'}))).rows.map(function(a){return a.tag;});
  var dsc=fafServerQuery(W(Object.assign({},big,{sort:k+'-desc'}))).rows.map(function(a){return a.tag;});
  ok('15 '+k+' changes what the server returns', JSON.stringify(asc)!==JSON.stringify(dsc));
});
ok('16 an unknown sortBy is rejected to the default, as a whitelist must',
   JSON.stringify(fafServerQuery(W(Object.assign({},big,{sort:'evil-asc'}))).rows.map(function(a){return a.tag;}))===
   JSON.stringify(fafServerQuery(W(Object.assign({},big,{sort:'tag-asc'}))).rows.map(function(a){return a.tag;})));
ok('17 the server clamps an over-range page', fafServerQuery(W(Object.assign({},big,{page:99}))).pageIndex===r.pageCount-1);
ok('18 and a negative page', fafServerQuery(W(Object.assign({},big,{page:-3}))).pageIndex===0);

// --- paging + sorting together: no duplicate, no omission, totals steady ---
ok('19 every row served exactly once across pages', (function(){
  var seen={},dup=false,q0=fafServerQuery(W(Object.assign({},big,{sort:'cost-desc'})));
  for(var i=0;i<q0.pageCount;i++){
    fafServerQuery(W(Object.assign({},big,{sort:'cost-desc',page:i}))).rows.forEach(function(a){
      if(seen[a.tag]) dup=true; seen[a.tag]=1; });
  }
  return !dup && Object.keys(seen).length===q0.totalCount;
})());
ok('20 page 1 desc holds the largest values', (function(){
  var q=fafServerQuery(W(Object.assign({},big,{sort:'cost-desc'})));
  var v=q.rows.map(function(a){return fafValue(a,'cost');});
  return JSON.stringify(v)===JSON.stringify(v.slice().sort(function(x,y){return y-x;}));
})());
function tot(h){var m=h.match(/aria-label="Total Net[^"]*"/);return m?m[0]:null;}
ok('21 the totals row is identical across pages and sorts', (function(){
  var a=tot(fafContent(W(Object.assign({},big,{size:'wide',view:'assets',sort:'tag-asc',page:0}))));
  var b=tot(fafContent(W(Object.assign({},big,{size:'wide',view:'assets',sort:'cost-desc',page:1}))));
  return a===b && a!==null;
})());
// --- the UI still says all seven sort ---
var h=fafContent(W({size:'wide',view:'assets'}));
ok('22 seven sort controls', (h.match(/data-faf="sort"/g)||[]).length===7);
ok('23 the pager says it refetches', has(fafContent(Object.assign({},big,{size:'wide',view:'assets'})),'refetches from the server'));
ok('24 exactly one active aria-sort', (fafContent(W({size:'wide',view:'assets',sort:'cost-desc'})).match(/aria-sort="(ascending|descending)"/g)||[]).length===1);
console.log('');
