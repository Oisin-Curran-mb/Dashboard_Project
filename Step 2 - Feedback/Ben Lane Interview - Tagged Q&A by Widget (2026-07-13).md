...
*Each question below is tagged with the widget it applies to, matching the file names in `Step 4 - Widget Final Design`. Questions not tied to a specific widget are tagged General. (Tags added 13.07.2026; escaped markdown from the original export has also been cleaned up here — content is unchanged.)*
