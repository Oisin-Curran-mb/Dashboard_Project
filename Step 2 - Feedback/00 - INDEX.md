...
| `Edward Eoff Interview - Tagged Q&A by Widget (2026-08-10).md` | Frozen transcript extract — **new, 2026-08-24** | Second SME interview, same tagged-per-widget format as the Ben Lane file. Covers **W09 Payroll Scheduled Time Off** (one row per day; group by home department not pay group, because approval security is department-scoped; leave-type icons must go since types are client-configurable; no rejected status exists; the coverage-overlap panel endorsed) and **W04 Remittance Pledges** (the widget is a *pledge* widget queried from the pledge table first — activity dates are cosmetic and a remittance with no pledge shows nothing; "Date Receipts Through" is really 1 Jan to the selected date; never label a date control "fiscal"; 500–800 pledges per activity is a real scale case). **Two caveats stated in the file:** the Teams VTT had no speaker tags so attribution is inferred from content, and the ASR mangled "remittance" throughout, so reconstructed wording is marked. Supersedes the Ben Lane answer on Q24 (pay group vs department). |
...
| `Interviews Transcripts/` | Subfolder | Raw interview transcripts as they come in. Contents: `2026-07-13 - Ben Lane SME Interview - Widget Usage (Most vs Least Used).docx`; `2026-08-10 - Edward Eoff SME Interview - Payroll Time Off + Remittance Pledges.vtt` and `.docx` *(added 2026-08-24 — the .vtt is the machine-readable copy, the .docx the human-readable one; keep both unedited, corrections belong in the tagged Q&A file, not here)*. |
| `Market Research/` | Subfolder — **new, 2026-07-21** | One file per widget (`WNN - Name.md`) for competitor/market research — how other companies present this kind of information, visuals and information included. Added because that research already existed but was split across `Step 3 - Mock_Work/Widget_Specs/` and `Step 4 - Widget Final Design/` per widget (e.g. W15's Hiline "runway" research), which made it easy to lose track of during a restructure. **All 16 files researched as of 2026-07-23** (W12 has no file by design); see the subfolder's own `00 - INDEX.md` for per-widget status. Files: `W01 - Budget Compared to Actual.md`, `W02 - Pension Plans.md`, `W03 - Payroll Distributions.md`, `W04 - Remittance Pledges.md`, `W05 - Receivable Invoices Outstanding.md`, `W06 - Insurance Billing Plans.md`, `W07 - Deposit Accounts.md`, `W08 - My Status.md`, `W09 - Payroll Scheduled Time Off.md`, `W10 - Loans With Balance Due.md`, `W11 - Fixed Asset Values.md`, `W13 - Purchasing Management.md`, `W14 - Main Content Tasks.md`, `W15 - Bank Balances.md`, `W16 - Accounts Payable By Due Date.md`, `W17 - Gifts Pledges.md`, plus `TEMPLATE.md`, `README.md`, `Chart Inventory.md`, `Dashboard Widget Reference.md` *(list added 2026-08-19 so the tracker-keeper audit recognises these as indexed)*. |
...
11. **`Market Research/` is the intended home for all per-widget competitor research going forward.** New market/competitor research for a widget belongs there (`WNN - Name.md`, one file per widget, no file for widget 12), not folded into `Widget_Specs/` or `Widget Final Design` docs as a fresh addition — those existing sections stay put until an explicit migration pass moves or copies them here. Don't do that migration silently; it touches Step 3 and Step 4 files too. Follow `Market Research/TEMPLATE.md`'s exact shape (What This Widget Shows Today, Data Used, Competitor/Industry Findings with a confidence read per source, Visual Options aiming for 3, Net Assessment, Sources) — including when using the `deep-research` skill to actually do the research, so all 16 files stay consistent as they get filled in one at a time.
