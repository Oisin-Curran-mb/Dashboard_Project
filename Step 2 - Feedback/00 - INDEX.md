...
| `Market Research/` | Subfolder — **new, 2026-07-21** | One file per widget (`WNN - Name.md`) for competitor/market research — how other companies present this kind of information, visuals and information included. Added because that research already existed but was split across `Step 3 - Mock_Work/Widget_Specs/` and `Step 4 - Widget Final Design/` per widget (e.g. W15's Hiline "runway" research), which made it easy to lose track of during a restructure. **Structure only as of 2026-07-21** — every file is currently an empty placeholder; see the subfolder's own `00 - INDEX.md` for status per widget. |
