...
> 
> **Question and answer from Edward Eoff (10.08.2026):** Reset-vs-persist still not answered — but Edward settled what the filter actually *means*, which was the bigger unknown. Verified live during the call: "Date Receipts Through" is really **1 January of that year through the selected date** — "it's just assuming a 1/1 to 8/1 date range." He offered three options going forward (keep assuming 1 January; derive the begin date from the earliest pledge begin date before the selection; or let the user set an explicit begin **and** end date, which he preferred: "that way they're just in full control over it… we don't have to assume anything"). **Terminology instruction, unprompted:** never label this "fiscal" — "that makes people think about their general Ledger fiscal years, and this has nothing to do with the general Ledger fiscal year." Use "pledge year to date" or "calendar year to date". See `Edward Eoff Interview - Tagged Q&A by Widget (2026-08-10).md`, Tag: W04.
...
> **ANSWERED 2026-08-25 (Feargal call) — this closes the question.** Remittances and gifts are **separate modules that can both pledge toward a purpose**, so the functional difference is primarily **the source or route of the contribution**, not the data or the audience. Two SME passes had failed to settle this (Ben described both the same way; Edward explained W04's model in depth but was never asked the comparison). Separately, and on top of that product answer, the project owner decided on 2026-08-24 to split the two by ROLE in the redesign: W04 answers "which pledges are not arriving as expected", W17 answers "how are we tracking against goal". Keep the two statements distinct — one is how the system works, the other is how we chose to present it. ⚠️ Source is AI-generated meeting notes with no transcript supplied, so treat the wording as paraphrase: `Feargal Call - Action List (2026-08-25).md`.
...
> Your answer:
> 
> **Question and answer from Edward Eoff (10.08.2026):** Not answered as a preference — Edward was asked whether a calendar exists today and confirmed **it does not**, framing it as brand-new functionality rather than a revamp of an existing view. He raised no objection to adding one, but never said a calendar beats a list. Still open as a preference question.
> 
> **Related and settled:** a multi-day request must render as **one row per day**, not a consolidated date range — "just to be consistent with the payroll module itself, you'd want to show one row per day and not do date ranges." Edward acknowledged a range reads better in real life and chose consistency anyway.
...
> 
> **Question and answer from Edward Eoff (10.08.2026):** Pulls the other way, from the approval side. Edward endorsed showing an approver who else has requested the same dates **before** they approve, in the approver's own voice: "I need one of you 2 for my team to stay… I can't have both you off same time." That is coverage/resourcing, not pre-payroll checking. Both readings now have SME support, which suggests the widget serves two jobs (the approver's coverage decision and the payroll run) rather than one — worth resolving rather than averaging.
...
> 
> **Question and answer from Edward Eoff (10.08.2026) — supersedes the above.** Answered directly, with a structural reason: **home department**, because that is what scopes approval rights. "The way you assign a supervisor over certain employees is handled by the home department." The deciding factor is cardinality — an employee belongs to exactly one home department but "could have belong in 10 different pay groups" (compensations can span pay groups), so pay group is a weak filter for "show me who is off". Edward explicitly acknowledged Ben's point that many clients don't populate home departments out of the box while all of them use pay groups, and resolved it as **"always default to department 1st and just have pay group as an option."** If pay group is offered, it must resolve to **current active** compensations only (active flag on the header, dates on the detail, most recent wins) — "make sure we're not pulling a pay group that they belong to 20 years ago."
...
**Still fully open, not touched in either interview pass:** Widgets 6 (Insurance Billing Plans — Q14/Q15/Q16), 12 (Empty Slot — Q29), ~~the exact functional difference between Gifts Pledges and Remittance Pledges (Q10/Q38)~~ **CLOSED 2026-08-25 by the Feargal call: separate modules, both able to pledge toward a purpose, so the distinction is the source/route of the contribution. See Q10 and Q38.** the AR aging band structure (Q12), individual-vendor amount display on AP (Q37 — the entry-vs-approval role split is now known, but not whether vendor-level detail belongs in this widget), grouping POs by department/vendor (Q31), empty states (Q41), and fiscal-period-specific date-filter defaults (Q40 — persisted filters are the general expectation, but no fiscal-period-specific default was confirmed; note Edward's instruction that pledge/remittance date controls must **never** be labelled "fiscal", as it collides with the GL fiscal year).

**Added after the Edward Eoff interview (10.08.2026) — answered there, not tied to a question in this tracker.** These are recorded in full in `Edward Eoff Interview - Tagged Q&A by Widget (2026-08-10).md` and are listed here so the tracker doesn't look like it missed them:

- **W09 — there is no rejected status, and no third status of any kind.** "It's either pending or it's approved. We really don't have a third status." A past-due pending request needs a coined label; Edward found "rejected" too strong, floated "unapproved", and explicitly handed the naming to Oisin, Ben or Feargal. **Still an open naming decision.**
- **W09 — ASKED BUT NEVER ANSWERED (follow-up needed):** can a request whose date has already passed still be approved, and does approving it late affect payroll? Oisin asked this at 12:29 of the Edward call; the conversation moved on to naming and never came back. This determines whether the Outstanding state is actionable or purely informational.
- **W09 — pay group is not a property of a time-off request.** It is derived from the employee's current compensations at read time and can only serve as an **employee filter**, never as a grouping dimension of requests — unlike home department, which sits on the employee record. Edward: "the time off request, of course, is also not specific to a pay group."
- **W09 — Edward's pay-group position is conditional, not a rejection.** He landed on "as long as you're doing all that cool stuff [resolving current active compensations], then I think pay group is fine… especially if Ben recommended that", before settling on department-as-default with pay group as an option. Don't cite him as having ruled pay group out.
- **W09 — per-type leave icons must go.** Leave types are client-configurable (Modify Employer → Time off hours: renameable descriptions, active checkboxes), so slot 1 is not reliably "Vacation" and a fixed icon will mislabel. Colour coding is fine.
- **W09 — approval happens in three places:** the employee record, the widget, and the portal.
- **W04 — the widget is a pledge widget, queried from the pledge table first**, then linked to remittance history detail. "I'm not going to actually show all remittances in the date range. It has to be hooked to the pledge." Activity begin/end dates are cosmetic ("just there to look pretty"); pledges dictate which activities display; a remittance with no pledge shows nothing at all (Edward tested this live). This reversed the direction the design had been heading. Schema notes: pledges live under **Manage Pledges**, one per church per activity, dated 1 Jan to year end; the **pledge detail table carries the activity ID** plus begin/end dates; the receipts link is to remittance history detail, but he hedged that table name — **confirm against the code**.
- **W04 — the date window does two jobs.** The 1 Jan → selected-date range both selects which pledges are in scope *and* bounds the receipts counted against them, not one or the other.
- **W04 — a free begin/end range would make the widget serve two use cases**, per Edward: pure receipts activity for a period (pledge/expected/% columns at zero), or full pledge progress when the range contains pledges. He judged the columns below would survive a free range.
- **W04 — scale warning:** customers may hold **roughly 500 to 800 pledges against a single activity** (⚠️ the figure is reconstructed from a garbled transcript line, "508 hundred" — confirm with Edward before sizing anything on it), so any activity-to-pledge drill needs top-N plus pagination. Agreed approach: cap the inline list around 10 to 20 and offer a "who is furthest behind" query beyond that.
- **W04 — unposted remittances are not expected money**, just entry in progress; they are only keyed once funds are actually received.
- **General — beta1 test data offer stands.** Edward will add 2026 pledges and remittances on request, and suggested the QA matrix: pledge with no remittance, remittance with no pledge, remittance correctly tied to a pledge.
