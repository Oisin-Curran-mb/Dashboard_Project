...
- **All-districts behaviour of grid/details when `districtId` is omitted.** RESOLVED: the grid and details apply the district filter only when a district is chosen (`if districtID != Guid.Empty`), so omitting it returns all districts aggregated (confirmed in code: gridPlans_NeedDataSource / Details, conditional WHERE DistrictID). The widget's default view is therefore a single call, and the client fan-out fallback is not needed.
...
- **Entitlement behaviour.** The gating right is the `/PensionBilling` Inquiry right, not a right called `PensionBenefits` (confirmed in code: DataPanelControl entitlement check, /PensionBilling). The legacy hides the widget and omits it from the picker for an unentitled user; recommend the modern API return an explicit 403 rather than an empty 200 that reads as "no pension data".
