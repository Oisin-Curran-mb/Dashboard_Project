...
- **Active-pledge inclusion at a date.** Confirm exactly which pledges are returned when `receiptsThrough` falls outside a pledge's term: only `Active = true` pledges whose `BeginDate <= receiptsThrough`, or also pledges that have already ended, so an ended-but-unpaid pledge still surfaces its outstanding balance.
