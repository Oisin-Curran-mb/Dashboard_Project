...
|---|---|---|---|---|---|
| `window` | enum | yes | `this_month` \| `this_period` \| `this_quarter` \| `this_year` \| `all_time` \| `custom` | none | this_month = calendar month containing `asOf`; this_period = the org's current fiscal period containing `asOf`; this_quarter = ROLLING, last 3 months back from `asOf`; this_year = ROLLING, last 12 months back from `asOf`; all_time = no lower bound, everything through `asOf`; custom = the `from`/`to` pair |
...
- **Orphaned `CompensationDistributionID` label behaviour:** what `distributionName` is returned when the org has deleted the distribution record (fallback label vs empty string).
