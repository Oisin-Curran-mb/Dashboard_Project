...
**Module:** Finance
**Status:** 🟢 Final design — locked · **v2.2 (2026-08-30): built Final, Jo design; `FC_VERSION[2]` = 2.2 in the build. First built as v2.0 on 2026-07-27.** Locked-doc rule: only version-tagged updates (v2, v3...) may modify this doc.
...
**Confluence dossier:** [Step 6 - Sign off document/Pension Plans](../Step%206%20-%20Sign%20off%20document/Pension%20Plans/) — Confluence pull 2026-07-27, page 7371030531, Parts A/B/C over 14 sections. **[v2.2 audit, 2026-08-30]** No `Reconciliation - Pension Plans.md` exists yet and this doc has no Sign-off Input (Jo) section, so every flag in that dossier is still Unreviewed.
**[v2.2 — 2026-08-30, owner instruction] The Districts roster view is CUT, and the appointees drill modal gives its width back to the table.** Two changes, both by direct owner decision.

**(1) Districts view removed — this supersedes v2.1 below and Feargal's B6 ask.** The **Districts** segment is gone from the view toggle, which is back to **Table / Pie / By district**. Removed with it: `penFDistTable`, `penFDistData`, `penFPeopleIn`, the `dist-open` and `export-dist` handlers, the `penDistOpen` state key, and the `penf`-scoped roster CSS. The `.prf-root` copies of `pen-dist-panel` and `pen-dist-foot` are deliberately left in place — W03's employee level uses them, so deleting them would break a different widget. **District survives as the top-left filter chip**, which is the part the owner asked to keep. A stale `penViewM === "dist"` falls back to the table with Table marked pressed (the `gpFDonut` rollback pattern), at both Explore and Detail. Jo's ported `index.html` never carried this view, so no port change was needed.

**(2) Appointees modal: table takes the majority, summary pinned far right.** The modal body was being laid out as a flex **row** by accident: `.pen-detail-modal .modal-b{display:block}` sits at char 144,515 and a later `.modal-wide .modal-b{...display:flex...}` at char 276,913 has equal specificity, so the later one won. The table therefore shrank to roughly 500px of a 960px modal — truncating "CHURCH / ORGANISA…", "ANNUAL AM…" and the appointee names — while its own footer was stranded beside it in the dead space. Now made deliberate rather than accidental, at specificity 0,3,0 (`.modal.pen-detail-modal ...`) so it genuinely wins rather than relying on which declarations the later rule happens to omit: body is `display:flex; align-items:flex-start; gap:16px`, the table is `flex:1 1 auto; min-width:0`, and `.pen-dtotal` is a 190px column pinned right with a left divider. **Modal width is unchanged at 960px** per the owner's "the popup needs to be the same size". No JS touched; no columns added or removed.

**Verified:** `final-check-rules.py` widget 2 — **0 HIGH**. DOM-shim driver — **23 assertions, 0 failures**, covering: the roster functions and state key are gone; the toggle has exactly 3 segments and no `dist` button; Table/Pie/By district all still render; `penFDistChip` still renders the district filter; a stale `dist` state neither throws nor renders a roster at either tier and marks Table pressed; the district filter still scopes the table; and the appointees modal still renders its four columns. **FC_VERSION[2] = 2.2.** ⚠️ Pixel layout is not machine-verifiable here — the widened table is a CSS cascade result confirmed by a specificity check, not a measured render. ⚠️ The same four-way `.modal-wide` conflict exists file-wide; only the pension modal is fixed.

**[v2.1 — 2026-08-25, Feargal call] Districts are now expandable, not just a filter.** ⚠️ **SUPERSEDED by v2.2 above — this view no longer exists.** Feargal asked to show pension amounts by district and let users expand a district to see the participating people, with that expanded information exportable. The widget already drilled plan to appointees, with district as a filter chip; this adds the other axis as a new **Districts** view: parent rows are districts with people count and annual amount, expanding inline to the people participating (Appointee / Church or organisation / Plan / Annual amount), with an **Export to Excel** on the expanded panel scoped to that district. Deliberately named "Districts" to avoid collision with the existing "By district" toggle segment, which is a grouped bar chart, not a roster. Reachable at both Explore and Detail. Verified: 51-assertion DOM-shim driver, 0 failures, including that district amounts and counts reconcile to the widget totals and that no person from another district leaks into an expanded panel. Export is a Rule 11 stub, no export backend in the mock. FC_VERSION[2] = 2.1.

**Last verified against build:** 2026-08-30 via widget-final-check-audit against `FC_VERSION[2]` = 2.2: `final-check-rules.py --widget 2` re-run, **0 HIGH** (11 MED, all pre-existing em dash and heuristic name-match notes), and every v2.2 code claim confirmed by reading the build (roster functions and state key gone, 3-segment toggle, stale `dist` falls back to Table, modal at 960px with the flex row now at specificity 0,3,0). Previous: 2026-07-27 via build-final-widget (Final, Jo design: 175-assertion Node driver + final-check-rules.py + CSS parse check + click-path test); 2026-07-20 via widget-final-check-audit (see 00 - INDEX.md)
...
| Plan dimension (the grouping key) | Dynamic list from the Pension Billing module | **[v2.2 audit, 2026-08-30]** The built Final groups by PLAN, not by an abstract plan-type category: org-configured plan names, which in the Final's dataset are Employee Before Tax-UMPIP, CRSP-DB-%, CRSP-DC, CPP and Flat CRSP-DB. No "Defined Benefit / Defined Contribution / 403(b)" category field exists in the legacy tables or the modern API, so the earlier illustrative values were never a real dimension and must not be specced as one. | [BUILD] + [DOC, Step 5 Pension Plans API Spec, Not in scope] |
...
Headline number's exact math (how the total is summed across districts/plans): **[v2.2 audit, 2026-08-30]** a client-side sum of the annual amounts on the appointee rows currently in scope, never a stored or server-computed total (`penFTotal` sums `penFFiltered`). The district chip is the only control that changes that scope; the plan rows are a roll-up of the same appointee rows, so the table footer, the pie centre and the Glance headline always agree by construction. Decimal-safe arithmetic is required: the Final's dataset deliberately carries a cent split (3,518.66 + 3,518.65 = 7,037.31) that float maths gets wrong. [BUILD] + [DOC, Step 5 Pension Plans API Spec]
...
> **[v2.2 audit, 2026-08-30] Superseded by the built Final.** The Final's plan table is fully user-sortable on every header (`penFRows`). The default is **annual amount descending**. A first click sorts Plan ascending, and Appointees and Annual amount descending; a second click on the same header toggles direction. Ties break on amount (when sorting by Appointees) or on plan name (when sorting by Annual amount). There is no fixed Church District alphabetical sort, because the Final's plan table has no District column at all: district is the filter chip, and the district dimension is read through the By district view and the drill instead.

*(v1, superseded by the block above, kept for history:)*
