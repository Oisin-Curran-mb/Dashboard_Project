...
**Confluence dossier:** [Budget Compared to Actual (Confluence pull 2026-07-27).html](../Step%206%20-%20Sign%20off%20document/Budget%20Compared%20to%20Actual/Budget%20Compared%20to%20Actual%20%28Confluence%20pull%202026-07-27%29.html) (Confluence page 7371554817; Parts A/B/C, 15 sections, including section 12 "Review of the W01 redesign" and flags F1 to F9). No `Reconciliation - Budget Compared to Actual.md` exists yet, so every one of those flags is still **Unreviewed** and none has been accepted, rejected or disputed.
...
**Last verified against build:** 2026-08-30 via widget-final-check-audit (unattended). The build is unchanged since the v2 tag: `FC_VERSION[1]` is still `2.0` and the Final Check tab carries no dated entry later than 2026-07-27, so the design itself did not move; the corrections in this round are stale doc text only. Previous: 2026-07-27 via build-final-widget (Final v2, Jo design: 159-assertion Node driver + final-check-rules.py + full-flow modal test); 2026-07-21 via widget-final-check-audit.
...
| Headline (Glance, Explore, Detail) | Same dataset | Signed $ variance, coloured by favourability, with a Favourable/Unfavourable pill, a keyboard-focusable info tooltip and a quiet percent of the window's budget. **No sparkline in the built Final (v2):** the line-graph sparkline function `w1KpiLineTile()` is only called from the A/B/C branches of `WRENDER[1]`, never from the `opt==='F'` Final branch (see Views, v2 block) | [BUILD] |
| KPI header strip, 4 tiles (YTD Budget · YTD Actual · Variance · % Used) | n/a | **Not in the built Final (v2).** Cut and replaced by the single variance headline above; listed here only so this table is not read as a live field requirement (see "What Got Cut", first entry) | [DOC, this doc, cut 2026-07-27] |
...
| Favourability / direction logic | The active scope's own account type | Green = favourable (over budget on income, under budget on expenses); the flip applies at every level (see Fine-Tuning Notes entry 2026-07-09, colour-logic fix). In the built Final (v2) the scopes are **Income accounts / Expense accounts / Special report** ("Custom Report" was the v1 name), and a special-report line whose accounts are genuinely mixed renders **neutral grey**, neither green nor red (see Interaction Spec, v2 block) | [BUILD] |
| Week and Day grains | GL data | In the built Final (v2) Week and Day are real grains of the Time Window Module, not a separate "Weekly" view (the v1 Period View row below is superseded). `GLSummary` is period-grain, so sub-period actuals need transaction-level querying and sub-period budgets are derived, not stored (see Sign-off Readiness row 3) | [TO CONFIRM, owner TBD] |
...
| Report name, Report line (Special report scope only) | Customer-created lists (Special Reports module) | In the built Final (v2) these are the two steps of the special-report modal, labelled "Report name" then "Report line"; Special Report Title and Line Description were the v1 names (see Filters, Dependent fields note) | [DOC, this doc, closed 2026-07-21] |
