...
**Module:** HR
**Status:** 🟢 Final design — locked (built 2026-07-30, Jo design, tagged v2.0 in the build; the build has since moved to **v2.4**, 2026-08-30, through the pie restoration and pie sizing rounds recorded in the changelog entries below). Locked-doc rule: the body below describes only the current final design. Superseded design thinking is not deleted, it is dated and moved to the "Design History (superseded)" section at the end of this doc.
...
**[v2.4 — 2026-08-30, per direct instruction] Pie sizing and padding, set from BROWSER MEASUREMENT.** (No separate v2.3 entry exists: v2.3 was the first attempt of this same-day sizing round and v2.4 its measured correction; the Widget_Specs history and the build's own `FC_VERSION[6]` note both record them as one combined entry.) v2.3 sized the donut per tier on Jo's own scale (250 Explore / 280 Detail). Measuring the rendered result with `Step 3 - Mock_Work/chart-fill-check.js` found three faults: Explore left **47% of its chart area empty** (donut 250px, legend box 558px for only 166px of legend text), Detail **truncated its plan names** below a ~1100px container as the legend box was crushed to 0, and the `@media (max-width:900px)` stacking rule **never fired**, because a widget's width comes from its card and not the viewport. Fixes: the legend is **capped per tier** (250px Explore, 200px Detail) with a min-width above its measured text so plan names can never truncate; the donut **grows into the remaining space** via flex-basis with min-width:0 and max-width:100%, reaching **320px at Explore** (up from 250) and 240px at Detail, and can neither clip nor overflow; **stacking is container-driven** via flex-wrap, replacing the dead media query; and leftover space is **centred on both axes**, since `align-items:flex-start` alone had pinned the chart to the top of the card. Measured after: Explore 320px donut with 104/104px vertical and 119/203px horizontal gutters, Detail 240px side by side with 84/84 and 15/49, and across container widths 1400 down to 600 **no truncation, no clipping and no lopsided gutters**. Verified: 0 HIGH static gate, 78-assertion driver 0 failures, chart-fill-check clean at five widths. ⚠️ The check measures width thoroughly and height only semi-automatically, and cannot resize a maximized browser window so it varies the card's width instead. No other widget was touched. `FC_VERSION[6]` = 2.4.

**[v2.2 — 2026-08-30, per direct instruction] The legacy PIE CHART is restored.** `insF` had no chart at all; Step 1 records the original widget as a side-by-side table and pie, so `insFPieChart` plus a **Table / Pie** segment were added. All four documented behaviours of the original chart are honoured: one segment per plan with at least one enrolment, labelled with the plan name; plans with **zero enrolments stay in the table but are not charted** (with a note stating how many and why, so table and chart never silently disagree); hovering a segment shows that plan's enrolment count; and **no drill-down**, which is why the legend rows are plain text rather than the clickable legend the pension donut uses. The type filter drives the pie as well as the table. **Tiers:** Detail shows the original side-by-side reading (table left, pie right); Explore carries the Table / Pie segment with **Table as the default**, since 6 columns cannot hold both legibly; Glance charts nothing. The toggle appears at Explore only, because Detail shows both at once. No maths was touched, and the pie's charted total equals the table's Total enrolled for the same filter. Verified: 0 HIGH on the static gate, 56-assertion DOM-shim driver, 0 failures. ⚠️ Visual result not machine-verified. `FC_VERSION[6]` = 2.2.
...
**Last verified against build:** 2026-09-02 via widget-final-check-audit (unattended continuation), completing the 2026-08-30 pass that was interrupted before it could stamp this line. Verified against build **v2.4** (`FC_VERSION[6]` = `'2.4'`) by reading `insFPieChart`, `insFViewToggle`, `insFContent`, `insFNestedTable`, `insFDetailHeadRow`, `insFDetailTotalRow`, `INSF_COLORS` and the v2.4 per-tier CSS directly: every claim in the v2.1/v2.2/v2.4 entries above matches the code (Table default at Explore via `w.insView||"table"`, toggle rendered only at the wide tier, Detail 58/42 split with a dividing rule, zero-enrolment plans filtered from the pie with an uncharted-plans note, per-segment `<title>` hover, plain-text `cursor:default` legend, amethyst shade scale, legend caps 250/200 and donut flex-basis 320/240 per tier, sortable nested table via `data-insf="ins-sort"`, "Total (all types)" footer). The v2.4 verification figures (78-assertion driver, chart-fill-check clean at five widths) are quoted from the Widget_Specs 2026-08-30 entry, not re-run in this pass. Previous: 2026-07-30 via build-final-widget (Final, Jo design: ~202-assertion Node driver, 0 failures + final-check-rules.py 0 HIGH + browser-faithful CSS parse, 0 dropped rules); before that, not yet audited.
...
**Net assessment:** this is a single-dimension dataset (enrollment count per plan), so both standard visualisations apply cleanly with no missing dimension. The Final ships Jo's dense, scannable table with a proportional Share column, and since v2.2 (2026-08-30) it also carries the legacy pie: an amethyst donut of enrolment by plan, side by side with the table at Detail and behind a Table / Pie segment at Explore. The "which plan dominates" reading is therefore available both as text (the Share column) and as the chart.
...
| Partial (some plans have zero enrollments) | A plan with zero enrolled still appears as a row with a zero count and 0% share (e.g. the Building / Property plan in the sample set); it is not hidden. [BUILD] Legacy behaviour: plans with zero enrollments appear in the table but not in the pie chart [DOC — Step 1 research]. The Final honours the same rule since v2.2: a zero-enrolment plan is listed in the table but is never charted in the pie, and the pie carries a note saying how many plans are uncharted and why, so the table and the chart never silently disagree. If the current filter leaves no plan with any enrolment, the pie renders a chart-empty state reading "No plan has an enrolment to chart for [type]" while the table still lists the zero-count rows. [BUILD] |
...
- **Table / Pie segment (Explore only):** at Explore the header carries a Table / Pie segment with **Table as the default**; switching view is an instant client re-render, never a fetch. Detail shows table and pie at once so it carries no segment, and Glance carries none. The insurance-type filter drives the pie as well as the table. Hovering a pie segment (or its legend row) shows the plan name, enrolment count and percent via a native title tooltip, matching the legacy hover behaviour. [BUILD]
...
The Final carries Jo's Insurance layout one-to-one: a **Glance KPI card** (total enrolled + plan-count pill + "employees and dependents" caption), a **sortable table**, the **insurance-type filter chip** (the only fetch), inline **amethyst Share bars**, and a totals footer row (**"Total (all types)"**, carrying total enrolled and total cost). There is no drill modal. On top of Jo's base, the Final adds the owner enhancements: the expandable **Type -> Plan nested table** (Explore and Detail), a **Cost total column**, and Jo's **Share column kept** alongside Cost. Since v2.2 (2026-08-30) it also carries the **legacy pie** (`insFPieChart`, an amethyst donut of enrolment by plan, with a plain-text legend and no drill): table left and pie right at **Detail**, and a **Table / Pie** segment at **Explore** with Table as the default. Glance charts nothing. [BUILD]

**Nested-table columns (Explore / Detail):** Insurance type / plan | Share of total | Enrolled | Cost (the Share header reads "Share of total" since v2.1). A small Explore width trim (Share bar 104px) lets the four columns fit at 592px. Glance is unchanged (the KPI card). [BUILD]
...
**Size behaviour:** three sizes only, per General Widget Design Rules Rule 12: **Glance / Explore / Detail**, no Small. Implemented via the `fc-fmode` mechanism, mapping Jo's kpi / wide / xwide layouts. Glance is the KPI card; **Explore** renders either the nested Type -> Plan table or the pie, one at a time behind the Table / Pie segment (Table default); **Detail** renders both at once, table left (58%) and pie right (42%), divided by a rule. [BUILD]
...
- **Pie chart (since v2.2):** the donut SVG carries `role="img"` with an `aria-label` stating the charted total and plan count ("Enrolment by plan, N enrolled across M plans"); every segment carries a `<title>` naming the plan, its count and its percent; each legend row states its count and percent as DOM text, so colour is never the only signal in the chart either; legend rows are plain text with `cursor:default` (no drill), matching the legacy behaviour. [BUILD]
