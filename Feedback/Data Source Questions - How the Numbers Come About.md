...
These are for a **technical/data subject-matter expert** — someone who knows the Shelby Financials database and the legacy widget calculation logic, not a UX/usage interview. That's a separate document: see `Specialist Questions.md` for the Ben Lane interview on how widgets are actually *used*. This one is about whether the numbers themselves are right, complete, and calculated the way we think they are.
...
*Questions prepared by Oisin Curran — July 2026, based on the Purpose docs in `Dashboard_Research/`, the locked design docs in `Widget Final Desgin/`, and the Developer Punch List in `Mock_work/`.*
