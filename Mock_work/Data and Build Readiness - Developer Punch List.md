...
> **Purpose of this doc:** for each of the 14 locked widget designs (`Widget Final Desgin/`), list every filter/field/view the design needs, and mark whether the underlying data actually supports it today. Cross-referenced against the verified data sources and formulas in `Dashboard_Research/` and the known Legacy-vs-Modern-API gaps found while comparing the two codebases.
>
...
>
> **Out of scope:** W08 (My Status) and W14 (Main Content Tasks) — still deferred, no final design exists yet to check against.
...
| Line Description (dependent field, shown when Account Type = Custom Report) | 🔴 Missing/unconfirmed | **Open question:** is this customer-created the same way as Special Report Title (needs the same kind of list endpoint), or a fixed/hard-coded list we maintain? Changes the build approach — needs a dev/data answer before build. Mock option list only for now. |
...
| Variance Bar / Waterfall / Data Table views | ✅ Available | All derive from the same confirmed Actual/Budget/Variance formula |
| KPI header strip (YTD Budget/Actual/Variance/%Used) | ✅ Available | Straightforward aggregation of already-confirmed fields |
| Drill-through to GL | 🔴 Missing | No target page/URL confirmed — navigation gap, not a data gap |
